...
namespace Ximo.Utilities
{
    ...
    internal class PrivateReflectionDynamicObject : DynamicObject
    {
        ...
        private static IDictionary<string, IProperty> GetTypeProperties(Type type)
        {
            ...
            foreach (var prop in type.GetTypeInfo().GetProperties(BindingFlags).Where(p => p.DeclaringType == type))
            {
                typeProperties[prop.Name] = new Property {PropertyInfo = prop};
            }

            // Now, add all the fields
            foreach (var field in type.GetTypeInfo().GetFields(BindingFlags).Where(p => p.DeclaringType == type))
            {
